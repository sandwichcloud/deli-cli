...
func (c *inspectCommand) Register(cmd *kingpin.CmdClause) {
	command := cmd.Command("inspect", "Inspect a user").Action(c.action)
	c.userID = command.Arg("projectID", "The user ID").String()
}
...
func (c *inspectCommand) action(app *kingpin.Application, element *kingpin.ParseElement, context *kingpin.ParseContext) error {
	...
	user, err := c.Application.APIClient.BuiltInAuth().Get(*c.userID)
	if err != nil {
		...
		if *c.raw {
			...
			for _, field := range structs.Fields(user) {
